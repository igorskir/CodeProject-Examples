...
namespace WPFSampleDemo.TypeConverter
{
    ...
    public class GeoPointItem
    {
        ...
        public static GeoPointItem Parse(string data)
        {
            ...
            catch (Exception ex) { throw new FormatException("Latitude value cannot be converted", ex); }

            try
            {
                lon = Convert.ToDouble(items[1]);
            }
            catch (Exception ex) { throw new FormatException("Longitude value cannot be converted", ex); }
